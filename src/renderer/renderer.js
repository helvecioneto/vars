...
function setupEventListeners() {
    ...
    // Listen for quiz solver toggle from main process (global shortcut: Ctrl+Shift+Q)
    window.electronAPI.onToggleQuizSolver(() => {
        toggleQuizSolver();
    });

    // Listen for quiz solver status updates
    window.electronAPI.quizSolver.onStatus(handleQuizSolverStatus);
 ...
 */
async function captureAndAnalyzeScreen(autoAction = null) {
    // Visual feedback
    if (elements.screenshotBtn) {
    ...
    try {
        ...
        if (autoAction) {
            updateStatus(`Auto-executing: ${autoAction}`, 'processing');
            processScreenshotAction(autoAction);
            return;
        }
...
/**
 * Toggle quiz solver on/off
 */
async function toggleQuizSolver() {
    if (isQuizSolverActive) {
        // Stop the quiz solver
        const result = await window.electronAPI.quizSolver.stop();
        if (result.success) {
            isQuizSolverActive = false;
            if (elements.quizBtn) {
                elements.quizBtn.classList.remove('quiz-active');
            }
            updateStatus('Quiz solver stopped', 'ready');
            console.log('[QuizSolver] Stopped');
        } else {
        ...
        // CHECK PROVIDER: If OpenAI, use Screen Capture + Answer mode instead of Auto-Solver
        if (config.provider && config.provider !== 'google') {
            // Show warning message first
            updateStatus('⚠️ OpenAI: Using One-Shot Mode. Switch to Gemini for full Auto-Solver.', 'warning');

            // Wait a moment for user to read, then execute
            setTimeout(() => {
                captureAndAnalyzeScreen('answers');
            }, 4000);
            return;
        }

        updateStatus('Starting quiz solver...', 'processing');
        const result = await window.electronAPI.quizSolver.start();
        if (result.success) {
            isQuizSolverActive = true;
            if (elements.quizBtn) {
                elements.quizBtn.classList.add('quiz-active');
            }
            // Warning for OpenAI - Show in status instead of alert
            if (result.provider && result.provider !== 'google') {
                updateStatus(`Quiz Assistant Active (Read-Only/OpenAI)`, 'recording');
            } else {
                updateStatus(`Quiz solver active (${result.tool})`, 'recording');
            }
        } else {
            updateStatus(`Error: ${result.error}`, 'error');
            console.error('[QuizSolver] Start failed:', result.error);
        }
    }
}

/**
 * Handle quiz solver status updates from main process
 * @param {object} data - Status data from quiz solver
 */
function handleQuizSolverStatus(data) {
    console.log('[QuizSolver] Status:', data);
    ...
    const formatStats = () => {
        if (data.stats) {
            const parts = [];
            if (data.stats.totalAnswered > 0) parts.push(`${data.stats.totalAnswered} answered`);
            if (data.stats.positionReuses > 0) parts.push(`${data.stats.positionReuses} cached`);
    ...
    const formatContext = () => {
        if (data.context?.hasLearnedPositions) {
            return ' ✓';
        }
        return '';
    };
    ...
    switch (data.status) {
        case 'started':
            updateStatus(`Quiz solver active (${data.tool})`, 'recording');
            break;

        case 'stopped':
            let stoppedMsg = 'Quiz solver stopped';
            if (data.stats?.totalAnswered > 0) {
                stoppedMsg += ` (${data.stats.totalAnswered} answered)`;
            }
            if (data.stats?.aiCalls > 0) {
                stoppedMsg += ` [${data.stats.aiCalls} AI calls]`;
            }
            updateStatus(stoppedMsg, 'ready');
            isQuizSolverActive = false;
            if (elements.quizBtn) {
                elements.quizBtn.classList.remove('quiz-active');
            }
        ...
        case 'scanning':
            updateStatus(`Scanning${formatContext()}...`, 'processing');
            break;

        case 'learning-layout':
            updateStatus('Learning quiz layout...', 'processing');
            break;

        case 'classifying':
            updateStatus('Classifying screen...', 'processing');
            break;
        ...
        case 'classified':
            ...
            updateStatus(`Detected: ${label}`, 'processing');
            break;

        case 'analyzing':
        case 'answering':
            updateStatus(`Analyzing${formatStats()}...`, 'processing');
            break;

        case 'openai-result':
            // Display valid Open AI answer
            updateStatus(`Answer: ${data.answer} (OpenAI Stopped)`, 'success');
            // Flash the button or UI to indicate success
            if (elements.quizBtn) {
                elements.quizBtn.classList.remove('quiz-active');
                setTimeout(() => elements.quizBtn.classList.add('quiz-success'), 100);
                setTimeout(() => elements.quizBtn.classList.remove('quiz-success'), 2000);
            }
            break;

        case 'quiz-detected':
            const quizTypeLabel = data.type ? ` [${formatQuizType(data.type)}]` : '';
            const cacheLabel = data.usingCache ? ' (cached)' : '';
            let statusMsg = `Quiz${quizTypeLabel}: ${data.correctAnswer}${cacheLabel}`;

            if (data.hasSubmitButton) statusMsg += ' → Submit';
            if (data.hasNextButton) statusMsg += ' → Next';

            updateStatus(statusMsg, 'processing');

            // Show response in content area
            if (data.question || data.correctAnswer) {
                let responseText = '';
                if (data.question) responseText += `**Pergunta:** ${data.question}\n\n`;
                responseText += `**Resposta:** ${data.correctAnswer}`;
                if (data.explanation) responseText += `\n\n**Explicação:** ${data.explanation}`;
        ...
        case 'clicking-submit':
            updateStatus('Clicking submit button...', 'processing');
            break;

        case 'clicking-next':
            updateStatus('Clicking next button...', 'processing');
            break;

        case 'clicking-finish':
            updateStatus('Clicking finish button...', 'processing');
            break;

        case 'quiz-completed':
            const completedMsg = data.questionsAnswered
                ? `Quiz completed! ${data.questionsAnswered} questions answered`
                : 'Quiz completed!';
            updateStatus(completedMsg, 'recording');
            break;

        case 'scrolling':
            updateStatus('Scrolling page...', 'processing');
            break;

        case 'clicking-continue':
            updateStatus('Clicking continue watching...', 'processing');
            break;

        case 'stuck-recovery':
            updateStatus('Stuck detected, recovering...', 'processing');
            break;

        case 'waiting-for-video':
            const silentIndicator = data.silent ? '(silent mode)' : '';
            updateStatus(`Video: ${data.seconds}s remaining ${silentIndicator}`.trim(), 'processing');
            break;

        case 'video-complete':
            updateStatus('Video finished, resuming scan', 'processing');
            break;

        case 'waiting':
            updateStatus(data.reason || 'Waiting for change...', 'processing');
            break;

        case 'no-quiz':
            updateStatus(`No quiz: ${data.reason || 'different content'}`, 'ready');
            break;

        case 'state-change':
            // Debug info - don't update status bar, just log
            console.log(`[QuizSolver] State: ${data.previousState} → ${data.state}`);
            break;
        ...
        default:
            // For any other status, just show it
